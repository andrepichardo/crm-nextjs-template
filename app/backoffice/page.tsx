...
export default async function BackofficeDashboard() {
  ...
  const wonDealValue = wonDeals.reduce((sum, deal) => sum + Number(deal.value), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard title="Total Contacts" value={contactsCount || 0} icon={Users} description="Active contacts" />
        <StatCard title="Companies" value={companiesCount || 0} icon={Building2} description="Registered companies" />
        <StatCard
          title="Active Deals"
          value={dealsCount || 0}
          icon={Briefcase}
        ...
        />
        <StatCard title="Open Tasks" value={tasksCount || 0} icon={CheckSquare} description="Pending tasks" />
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">
                {deals && deals.length > 0 ? ((wonDeals.length / deals.length) * 100).toFixed(1) : 0}%
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Total Won Value</span>
              <span className="text-sm text-muted-foreground">${wonDealValue.toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">Average Deal Size</span>
              <span className="text-sm text-muted-foreground">
                ${deals && deals.length > 0 ? (totalDealValue / deals.length).toFixed(0) : 0}
              </span>
            </div>
          </CardContent>
        </Card>
